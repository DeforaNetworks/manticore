...
class NetBSD(Platform):
    '''
    A simple NetBSD Operating System Platform.
    This class emulates the most common NetBSD system calls
    '''
    ...
    def __init__(self, program, argv=None, envp=None, disasm='capstone', **kwargs):
        ...
        '''
        super(NetBSD, self).__init__(path=program, **kwargs)

        self.program = program
        self.clocks = 0
        self.files = []
        self.syscall_trace = []
        # Many programs to support SNetBSD
        ...
        if program is not None:
            ...
            self.arch = {'x64': 'amd64'}[self.elf.get_machine_arch()]
    ...
    def _init_cpu(self, arch):
        ...
        self.procs = [cpu]
        self._current = 0
        self._function_abi = CpuFactory.get_function_abi(cpu, 'netbsd', arch)
        self._syscall_abi = CpuFactory.get_syscall_abi(cpu, 'netbsd', arch)
    ...
    def _mk_proc(self, arch):
        mem = Memory32() if arch in {'i386'} else Memory64()
    ...
    def __getstate__(self):
        state = super(NetBSD, self).__getstate__()
    ...
    def __setstate__(self, state):
        ...
        """
        super(NetBSD, self).__setstate__(state)
    ...
    def sys_newuname(self, old_utsname):
        ...
        info = (('sysname', 'NetBSD'),
                ('nodename', 'Amnesiac'),
                ('release', '7.1_STABLE'),
                ('version', 'NetBSD 7.1_STABLE (GENERIC)'),
    ...
    def syscall(self):
        ...
        try:
            table = getattr(netbsd_syscalls, self.current.machine)
    ...
    def sys_fstat64(self, fd, buf):
        '''
        Determines information about a file based on its file descriptor (for NetBSD 64 bits).
        :rtype: int
        :param fd: the file descriptor of the file that is being inquired.
        :param buf: a buffer where data about the file will be stored.
        :return: C{0} on success, EBADF when called with bad fd
    ...
    def sys_stat64(self, path, buf):
        '''
        Determines information about a file based on its filename (for NetBSD 64 bits).
        :rtype: int
    ...
    def _arch_specific_init(self):
        assert self.arch in {'amd64'}

        self._uname_machine = self.arch
...
class SNetBSD(NetBSD):
    """
    Builds a symbolic extension of a NetBSD OS
    ...
    def __init__(self, programs, argv=None, envp=None, symbolic_files=None,
        ...
        super(SNetBSD, self).__init__(programs,
                                     argv=argv,
                                     envp=envp,
                                     disasm=disasm)
    ...
    # marshaling/pickle
    def __getstate__(self):
        state = super(SNetBSD, self).__getstate__()
    ...
    def __setstate__(self, state):
        ...
        super(SNetBSD, self).__setstate__(state)
    ...
    def _sys_open_get_file(self, filename, flags):
        if filename in self.symbolic_files:
            ...
            f = super(SNetBSD, self)._sys_open_get_file(filename, flags)
    ...
    def _transform_write_data(self, data):
        ...
        return super(SNetBSD, self)._transform_write_data(concrete_data)
    ...
    def sys_exit_group(self, error_code):
        if issymbolic(error_code):
            ...
            return super(SNetBSD, self).sys_exit_group(error_code)
    ...
    def sys_read(self, fd, buf, count):
        ...
        return super(SNetBSD, self).sys_read(fd, buf, count)
    ...
    def sys_write(self, fd, buf, count):
        ...
        return super(SNetBSD, self).sys_write(fd, buf, count)
    ...
    def sys_recv(self, sockfd, buf, count, flags):
        ...
        return super(SNetBSD, self).sys_recv(sockfd, buf, count, flags)

    def sys_accept(self, sockfd, addr, addrlen, flags):
        #TODO(yan): Transmit some symbolic bytes as soon as we start. 
        # Remove this hack once no longer needed.

        fd = super(SNetBSD, self).sys_accept(sockfd, addr, addrlen, flags)
        if fd < 0:
            return fd
    ...
    def sys_open(self, buf, flags, mode):
        ...
        rv = super(SNetBSD, self).sys_open(buf, flags, mode)
    ...
    def sys_openat(self, dirfd, buf, flags, mode):
        ...
        return super(SNetBSD, self).sys_openat(dirfd, buf, flags, mode)
    ...
    def sys_getrandom(self, buf, size, flags):
        ...
        return super(SNetBSD, self).sys_getrandom(buf, size, flags)
